      ...
      <div class="col-md-3 mb-4 ps-0 pe-md-2">
        <div class="profile-menu p-2 p-md-0" id="myTab" role="tablist">
          <ul class="list-unstyled d-flex flex-row gap-2 flex-md-column overflow-auto">
            ...
            <li class="p-md-3 d-flex align-items-center mb-md-5" id="order-tab" data-bs-toggle="tab" data-bs-target="#order-tab-pane" type="button" role="tab" aria-controls="order-tab-pane">
      ...
      <div class="col-md-9">
        <div class="tab-content pe-0 ps-md-2" id="myTabContent">
                  ...
                  <tbody>
                    @foreach($client->bookings as $b => $booking)
                        <tr>
                            ...
                            <td>{{ $booking->total_price }} <span class="small">{{ __('services.SAR') }}</span></td>
                            <td>{{ $booking->paid_amount }} <span class="small">{{ __('services.SAR') }}</span></td>
                            <td>{{ ucfirst($booking->booking_status) }}</td>
                            <td><a href="{{route('service.booking.show', $booking->id)}}" class="btn border rounded"><i class="fa-solid fa-arrow-right text-white"></i></a></td>
                        </tr>
                    @endforeach
                  </tbody>
                </table>
          ...
          <div class="tab-pane fade" id="order-tab-pane" role="tabpanel" aria-labelledby="order-tab">
            <div class="table-profile">
              <div class="table-header">
                <h5 class="mb-0"><i class="fa-solid fa-clock-rotate-left"></i> @lang('site.profile_table_order_title')</h5>
              </div>
              <div class="table-responsive">
                <table class="table">
                  <thead>
                  ...
                  <tbody>
                    @foreach($client->orders as $o => $order)
                      <tr>
                        ...
                        <td>{{ $order->total_price }} <span class="small">{{ __('services.SAR') }}</span></td>
                        <td>{{ ucfirst($order->status) }}</td>
                        <td><a href="{{route('service.order.show', $order->id)}}" class="btn border rounded"><i class="fa-solid fa-arrow-right text-white"></i></a></td>
                      </tr>
                   @endforeach
                  </tbody>
                </table>
